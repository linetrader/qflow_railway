// FILE: /src/app/admin/level/policies/view/PoliciesTable.tsx
"use client";

import { useMemo } from "react";
import type { LevelPolicyListItem } from "@/types/admin/level-policies";

export type TableActions = {
  onEdit: (item: LevelPolicyListItem) => void;
  onDelete: (item: LevelPolicyListItem) => void;
};

export type Props = {
  items: LevelPolicyListItem[] | null;
  loading: boolean;
  onRefresh: () => void;
  ...
  const rows = useMemo(
    () =>
      (items ?? []).map((p) => ({
        ...
        name: p.name,
        levelsCount: String(p.levelsCount),
      ...
      <div className="card-body">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold">Level Policies</h2>
        ...
        <div className="overflow-x-auto">
          <table className="table w-full">
            <thead>
              <tr>
                <th className="text-left min-w-[140px]">Active</th>
                <th className="text-left min-w-[140px]">Name</th>
                <th className="text-right min-w-[140px]">#Levels</th>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="text-right">{r.levelsCount}</td>
